...
export interface Cierre {
  ...
  fecha: string
  alcance: string
  esperado: number    // ventas en efectivo del alcance
...
const live = makeLive<Cierre>(async () => {
  const { data, error } = await supabase.from('cash_closings')
    .select('id, fecha, alcance, esperado, fondo, contado, diferencia, motivo, usuario, created_at')
...
let seq = 0
export function registrarCierre(input: { fecha: string; alcance: string; esperado: number; fondo: number; contado: number; motivo: string | null; usuario: string }): Cierre {
  seq += 1
  // El cajón físico = fondo + ventas en efectivo, así que se compara contra la suma.
  const c: Cierre = { id: hasSupabase ? (globalThis.crypto?.randomUUID?.() ?? `cc-${seq}`) : `cc-${seq}`, ...input, diferencia: input.contado - (input.esperado + input.fondo), created_at: new Date().toISOString() }
  ...
  if (hasSupabase) {
    supabase.from('cash_closings').insert({ id: c.id, fecha: input.fecha, alcance: input.alcance, esperado: input.esperado, fondo: input.fondo, contado: input.contado, diferencia: c.diferencia, motivo: input.motivo, usuario: input.usuario, created_by: currentUserId() })
