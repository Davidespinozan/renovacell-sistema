...
const live = makeLive<Refund>(async () => {
  const { data, error } = await supabase.from('refunds')
    .select('id, order_id, tipo, monto, motivo, metodo, usuario, created_at')
...
export interface RegistrarInput { orderId: string; tipo: RefundTipo; monto: number; motivo: string; usuario: string }
